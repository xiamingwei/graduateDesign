...
<script>
  ...
  export default {
    ...
    methods: {
      getData() {
        this.service.getTopic1CommentsData().then((response) => {
      ...
      show(rows) {
        this.$Modal.info({
          ...
          content: `<span class="bold">楼主为:</span>${rows.owner}<br><span class="bold">楼层为:</span>${rows.floor}<br><span class="bold">层主为:</span>${rows.floorer}
           <br><span class="bold">内容为:</span>${rows.contents}<br><span class="bold">时间为:</span>${rows.time1}<br><span class="bold">发帖日期为:</span>${rows.postMessage}
           <br><span class="bold">时间为:</span>${rows.time2}<br><span class="bold">评论随时间的变化为:</span>${rows.commentsDate}
        ...
        if (this.query) {
          this.datas.map((item) => {
            if (item.owner.indexOf(this.query) > -1) {
              ...
              arr.push(item)
            } else if (item.time1.indexOf(this.query) > -1) {
              arr.push(item)
            } else if (item.postMessage.indexOf(this.query) > -1) {
              arr.push(item)
            } else if (item.commentsDate.indexOf(this.query) > -1) {
              arr.push(item)
